% 设置文件路径
data_file = 'C:\Users\wangjunwen\Desktop\article2\code\MCSs\data\Mtraining+Mtest\MCSs_train&test_data_3d.mat';
...
grid on;
title('3D Trajectory - Rosser_train&validate_data1', 'Interpreter', 'none');
xlabel('x');
ylabel('y');
zlabel('z');
view(45, 30); % 设置固定视角
...
plot(t, Mtraining(1:5000, 1), 'Color', x_color, 'LineWidth', 1.5);
title('3个维度的前200个时间步');
xlabel('t');
ylabel('x');
grid on;
...
plot(t, Mtraining(1:5000, 2), 'Color', y_color, 'LineWidth', 1.5);
xlabel('t');
ylabel('y');
grid on;

subplot('Position', [0.55, 0.1, 0.4, 0.25]);
plot(t, Mtraining(1:5000, 3), 'Color', z_color, 'LineWidth', 1.5);
xlabel('t');
ylabel('z');
grid on;

disp('图像绘制完成，并显示初始点坐标。');
